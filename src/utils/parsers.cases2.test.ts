...
import { parseCases2, parse2, type QaCase2 } from './parsers.cases2';
...
describe('parseCases2 — 24forcare', () => {
	it('извлекает вопрос + variants + answers + idx из h3 + p > strong', () => {
		...
		const cases = parseCases2('24forcare', div);
	...
	it('возвращает пустой массив если нет вопросов', () => {
		...
		expect(parseCases2('24forcare', div)).toEqual([]);
	});
});

describe('parseCases2 — rosmedicinfo layout1 (h3 + span highlighted)', () => {
	it('извлекает правильные варианты по жёлтому фону', () => {
		...
		const cases = parseCases2('rosmedicinfo', div);
...
describe('parseCases2 — rosmedicinfo layout2 (p.MsoNormal + br + плюс)', () => {
...
describe('parseCases2 — idx уникален в пределах массива', () => {
...
describe('parseCases2 — rosmedicinfo реальные примеры', () => {
...
const mkModel = (items: Array<Omit<QaCase2, 'idx'>>): QaCase2[] =>
	items.map((c, idx) => ({ ...c, idx }));

describe('parse2 — базовый матчинг по вопросу', () => {
	...
	it('точное совпадение вопроса — возвращает variants + answers case\'а', () => {
		...
		const res = parse2(model, 'Цель сестринского процесса', ['a', 'b', 'c']);
	...
	it('регистронезависимый матч (нормализация к lowercase)', () => {
		...
		const res = parse2(model, 'цель сестринского ПРОЦЕССА', ['a']);
		expect(res).not.toBeNull();
	...
	it('матч через includes когда вход короче сохранённого вопроса', () => {
		...
		const res = parse2(model, 'методы диагностики применяются при кардиомиопатии', ['ЭхоКГ']);
		expect(res).not.toBeNull();
	...
	it('матч через includes когда вход длиннее (префикс «N.» из источника)', () => {
		...
		const res = parse2(model, 'В патогенезе синдрома Рейно ключевую роль играет', ['a']);
		expect(res).not.toBeNull();
	});
	...
	it('нормализация пробелов (\\n, &nbsp;, табы схлопываются)', () => {
		...
		const res = parse2(model, 'Вопрос с множественными пробелами', ['a']);
		expect(res).not.toBeNull();
	});
	...
	it('нормализация тире (em-dash → hyphen)', () => {
		...
		const res = parse2(model, 'АД - что это', ['a']);
		expect(res).not.toBeNull();
	});
});

describe('parse2 — нет совпадений', () => {

	it('пустая модель → null', () => {
		expect(parse2([], 'любой вопрос', ['a'])).toBeNull();
	});
	...
	it('ни один вопрос не похож → null', () => {
		...
		const res = parse2(model, 'Совсем другая тематика про эндокринологию', ['x']);
		expect(res).toBeNull();
	});
	...
	it('similarity ниже SIMILARITY_THRESHOLD (0.85) → null', () => {
		...
		const res = parse2(model, 'Диагностика пневмонии у детей', ['x']);
		expect(res).toBeNull();
	});
});

describe('parse2 — fuzzy по Dice similarity (≥ 0.85)', () => {
	...
	it('небольшая опечатка (одна буква) — матчится', () => {
		...
		const res = parse2(model, 'Морфологичесий субстрат ЛГ', ['a']);   // «опечатка»
		expect(res).not.toBeNull();
	});
});

describe('parse2 — фильтрация по variants при коллизии вопроса', () => {
	...
	it('два case\'а с одинаковым вопросом — выбирает с максимальным overlap вариантов', () => {
		...
		const res = parse2(model, 'Какой препарат?', ['metformin', 'insulin']);
		expect(res).not.toBeNull();
	...
	it('частичный overlap variants — побеждает тот, у кого совпадений больше', () => {
		...
		const res = parse2(model, 'Q', ['a', 'b', 'c']);
		expect(res).not.toBeNull();
	...
	it('overlap через includes (входной вариант — префикс сохранённого)', () => {
		...
		const res = parse2(model, 'Q', ['ремоделирование правого']);
		expect(res).not.toBeNull();
	...
	it('ни один вариант не пересекается — выбирается кандидат с лучшим score по вопросу', () => {
		...
		const res = parse2(model, 'Длинный вопрос про медицину', ['xxx']);
		expect(res).not.toBeNull();
...
describe('parse2 — confidence score', () => {
	...
	it('частичный variants overlap → score пропорционально понижается', () => {
		...
		const resFull    = parse2(model, 'Q long enough for includes', ['aaa', 'bbb', 'ccc'])!;
		const resHalf    = parse2(model, 'Q long enough for includes', ['aaa', 'zzz'])!;
		const resNone    = parse2(model, 'Q long enough for includes', ['xxx', 'yyy'])!;
	...
	it('fuzzy question → score < 1', () => {
		...
		const res = parse2(model, 'Морфологичесий субстрат ЛГ', ['a'])!;
		expect(res.score).toBeLessThan(1);
		expect(res.score).toBeGreaterThan(0);
	});
});

describe('parse2 — устойчивость к шуму на коротких вариантах (регрессия)', () => {
	...
	it('числовой вариант «3» не матчится к «3 стадии» через includes', () => {
		...
		const res = parse2(model, 'Q', ['3 стадии']);
		expect(res).not.toBeNull();
	...
	it('однобуквенный вариант не раздувает overlap короткими подстроками', () => {
		...
		const res = parse2(model, 'Q long enough', ['полный длинный текст']);
		expect(res).not.toBeNull();
		expect(res!.answers).toEqual(['полный длинный текст']);
	});
});

describe('parse2 — тай-брейкер по idx (стабильность)', () => {
	...
	it('при равенстве overlap и qScore выигрывает меньший idx', () => {
		...
		const res = parse2(model, 'Q long enough match', ['x', 'y', 'z']);
		expect(res).not.toBeNull();
		expect(res!.answers).toEqual(['y']);   // idx=2 выиграл → его source-answer 'y' нашёлся во входных
	});
});

describe('parse2 — интеграция с parseCases2 (реальный HTML)', () => {

	it('находит вопрос в результатах parseCases2 по тексту', () => {
		const div = createDiv(`
			<h3>13. Морфологический субстрат ЛГ</h3>
			<p><span style="background-color:#fbeeb8;">1) ремоделирование правого желудочка; +</span> <br>2) гипертрофия и дилатация левого желудочка; <br>3) миокардит; <br>4) отек головного мозга; <br>5) увеличение селезенки.</p>
		`);

		const model = parseCases2('rosmedicinfo', div);
		const userVariants = [
		...
		const res = parse2(model, 'Морфологический субстрат ЛГ', userVariants);
	...
	it('при дублях от разных extractor\'ов фильтр по variants даёт правильный case', () => {
		...
		const res = parse2(model, 'Вопрос с двойной разметкой', ['1) правильный', '2) неправильный', '3) тоже нет']);
		expect(res).not.toBeNull();
...
describe('баг репорт', () => {
	...
	it('Большие языковые модели и генеративный искусственный интеллект', () => {
		...
		const res = parse2(model, question, userVariants);
