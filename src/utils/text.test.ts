...
describe('fn cleanTopic', () => {
	...
	it('одинаково нормализует год с пробелами и без них', () => {
		const compact = 'Сахарный диабет у детей-2020';
		const withTestType = `${compact} - Контрольное тестирование`;

		expect(cleanTopic(compact)).toBe('Сахарный диабет у детей - 2020');
		expect(cleanTopic(withTestType)).toBe('Сахарный диабет у детей - 2020');
	});
