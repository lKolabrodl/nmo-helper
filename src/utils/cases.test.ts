...
describe('баг репорт', () => {
	...
	it('Инфекция, специфичная для перинатального периода (по утвержденным клиническим рекомендациям) - 2025', () => {
		const div = createDiv(`
			<h3>6. В общем анализе крови у ребенка с перинатальной инфекцией на 3 - 5 сутки жизни отмечается</h3>
			<p>1) нейтрофилез &gt;5-7*10/9 л; <br>2) нейтрофилез &gt;4-5*10/9 л; <br>3) нейтрофилез &gt;20*10/9 л; <br><span style="background-color:#fbeeb8;">4) нейтрофилез &gt;7-9*10/9 л.</span></p>
			<p><br></p>
			<h3>7. В общем анализе крови у ребенка с перинатальной инфекцией на 3 - 5 сутки жизни отмечается</h3>
			<p>1) нейтропения &lt;1700 в 1мкл; <br>2) нейтропения &lt;1000 в 1мкл; <br><span style="background-color:#fbeeb8;">3) нейтропения &lt;2000 в 1мкл;</span> <br>4) нейтропения &lt;5000 в 1мкл.</p>
		`);

		const model = extractCases('rosmedicinfo', div);

		const question = 'В общем анализе крови у ребенка с перинатальной инфекцией на 3 – 5 сутки жизни отмечается';

		const userVariants = [
			"нейтропения <1700 в 1мкл",
			"нейтропения <1000 в 1мкл",
			"нейтропения <5000 в 1мкл",
			"нейтропения <2000 в 1мкл"
		]
		...
		expect(res).not.toBeNull();
		expect(res?.answers).toHaveLength(1);
		expect(res?.answers?.[0]).toContain('нейтропения <2000 в 1мкл');
	});
